...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_fed_loc"];
_fed_loc = getmarkerpos "fed_reserve";
_building = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
if(isNull _building) exitWith {};
if(!(_building isKindOf "House_F")) exitWith {hint "You are not looking at a house door."};
if(isNil "life_boltcutter_uses") then {life_boltcutter_uses = 0;};
if((nearestObject [_fed_loc,"Land_Dome_Big_F"]) == _building OR (nearestObject [_fed_loc,"Land_Cargo_House_V3_F"]) == _building) then {
...
switch (typeOf _building) do {
	...
	case "Land_Cargo_House_V3_F": {_cpRate = 0.0015;};
