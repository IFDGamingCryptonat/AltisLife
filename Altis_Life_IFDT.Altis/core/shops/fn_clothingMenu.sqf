...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart","press"] && playerSide != civilian) exitWith {hint "You need to be a civilian to use this store!"; closeDialog 0;};
if((_this select 3) == "reb" && (__GETC__(life_rebellvl) < 1)) exitWith {hint "You are not a rebel, piss off!"; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "You need to be a cop to use this store!"; closeDialog 0;};
if((_this select 3) in ["admin"] && (__GETC__(life_adminlevel) < 1)) exitWith {hint "You need to be a cop to use this store!"; closeDialog 0;};
if((_this select 3) in ["donator"] && (__GETC__(life_donator) < 1)) exitWith {hint "You need to be a donator to use this store!"; closeDialog 0;};
