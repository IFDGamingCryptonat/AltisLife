...
[] spawn
{
	private["_bp","_load","_cfg","_vest","_cfgv","_load2"];
	while{true} do
	{
		...
		_cfgv = getNumber(configFile >> "CfgVehicles" >> (vest player) >> "maximumload");
		_load = round(_cfg / 8);
		_load2 = _load + round(_cfgv / 8);
		life_maxWeight = life_maxWeightT + _load2;
