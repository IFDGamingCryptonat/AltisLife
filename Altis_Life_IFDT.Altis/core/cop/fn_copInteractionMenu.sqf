...
private["_display","_curTarget","_Btn1","_Btn2","_Btn3","_Btn4","_Btn5","_Btn6","_Btn7","_Btn8","_fed_loc"];
if(!dialog) then {
...
_fed_loc = getmarkerpos "fed_reserve";

if(_curTarget isKindOf "House_F") exitWith {
	if((nearestObject [_fed_loc,"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [_fed_loc,"Land_Cargo_House_V3_F"]) == _curTarget) then {
...
if(!((player distance (getMarkerPos "jail_drop") < 30)/* OR  (player distance (getMarkerPos "police_hq_2") < 30) OR (player distance (getMarkerPos "cop_spawn_3") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30)*/)) then 
{
