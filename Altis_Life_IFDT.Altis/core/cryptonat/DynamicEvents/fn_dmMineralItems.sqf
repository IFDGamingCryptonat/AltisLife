...
private["_dmVehicle","_dmItem","_itemWeight","_maxWeightRaw","_maxWeight","_baseAmount","_maxFit","_amount","_totalWeight"];
_veh = _this select 0;
_dmVehicle = _this select 1;
_dmItem = _this select 2;

/*
_baseAmount = 10; //initial inventory amount
_itemWeight = [_dmItem] call life_fnc_itemWeight; //what does the mineral weigh
_maxWeight = [_dmVehicle] call life_fnc_vehicleWeight; //how much can the vehicle hold total
_maxFit = floor((_maxWeight / _itemWeight) - _baseAmount); //how much of this item can the trunk hold
_amount = round(random _maxFit) + _baseAmount; //how much are we putting in the trunk
_totalWeight = _amount * _itemWeight; //weight calculation for the item insert script
_dmVehicle setVariable ["Trunk",[[[_dmItem,_amount]],_totalWeight],true];
*/
